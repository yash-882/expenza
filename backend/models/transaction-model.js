...
const transactionSchema = mongoose.Schema({
    ...
    values:  ['food','transport','housing','entertainment','healthcare',
              'education','others', 'salary', 'freelance', 'bonus'],
...
transactionSchema.pre('save', function (next){
    ...
    // Set that contains categories but with emoji
    const categoriesSet = ['🍽️ food','🚌 transport','🏠 housing','🎮 entertainment','💊 healthcare',
        '🎓 education','📦 others', '💵 salary', '💻 freelance', '💰 bonus'];
        
        
        // Appending emoji from begining of a category
        const category = categoriesSet.find(categ => categ.split(' ')[1] === this.category);
